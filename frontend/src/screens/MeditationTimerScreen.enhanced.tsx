...
export const MeditationTimerScreen: React.FC<MeditationTimerScreenProps> = ({
  ...
  const loadBellSound = async () => {
    try {
      // Note: You'll need to add a bell sound file to assets/sounds/
  ...
  const handleStart = async () => {
    ...
    if (state === 'idle') {
      ...
      if (selectedSound) {
        await ambientSoundService.play(selectedSound);
      }
      await startSession();
      await playBellSound();
  ...
  const handleComplete = async () => {
    ...
    if (selectedSound) {
      await ambientSoundService.fadeOut(3000);
    }
    ...
    // Show reflection prompt after a delay
    setTimeout(() => {
  ...
  return (
    <View style={styles.container}>
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        {/* Ambient Sound Selector (only show when idle) */}
        {state === 'idle' && (
        ...
        {state === 'idle' && !meditation?.duration && (
          <DurationPicker
            durations={DURATIONS}
